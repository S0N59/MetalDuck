...
@MainActor
protocol RenameProfileViewControllerDelegate: AnyObject {
...
/// A view controller for the "Edit Profile" modal.
/// This modal allows users to rename an existing custom profile.
final class RenameProfileViewController: NSViewController {
    weak var delegate: RenameProfileViewControllerDelegate?
    private let initialName: String
    
    private var clickMonitor: Any?
    
    private let titleLabel: NSTextField = {
        let tf = NSTextField(labelWithString: "Edit Profile")
    ...
    private let subtitleLabel: NSTextField = {
        let tf = NSTextField(labelWithString: "Enter a new name for this profile.")
    ...
    private lazy var saveButton: ModalActionButton = {
        let btn = ModalActionButton(title: "Save", style: .primary)
        btn.setAction(#selector(handleRename), target: self)
        return btn
    }()
    ...
    override func loadView() {
        ...
        let buttonsStack = NSStackView(views: [cancelButton, saveButton])
    ...
    @objc private func handleRename() {
        ...
        if !name.isEmpty {
            delegate?.renameProfileViewController(self, didRenameProfileWithName: name)
        }
    }
    
    @objc private func handleCancel() {
        delegate?.renameProfileViewControllerDidCancel(self)
    }
}
